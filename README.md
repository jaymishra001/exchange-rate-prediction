...
16. [Model Artifact](#16-model-artifact)
17. [Repository Structure & How to Run](#17-repository-structure--how-to-run)
...
The final XGBoost model (Section 10.4, corrected 7-feature set) is persisted with `joblib` so it can be reused without retraining:

```python
import joblib
joblib.dump(xgb_model, 'models/xgb_eurusd_model.pkl')
```

`joblib` is used instead of raw `pickle` — it's the standard convention for sklearn/XGBoost-family models, since it handles the large internal NumPy arrays (split thresholds, leaf values) more efficiently.

**`predict.py`** loads the model and exposes a single function for inference:
...
Run directly to see a worked example:
```bash
python predict.py
```

**⚠️ Version sensitivity:** pickled models are tied to the library versions they were trained with — loading `xgb_eurusd_model.pkl` with a different installed version of `xgboost`/`scikit-learn` can fail silently or throw an error. `requirements.txt` pins exact versions (not just package names) for this reason; always install from it before loading the model, rather than assuming the latest version will work.

---

## 17. Repository Structure & How to Run
...
├── models/
│   └── xgb_eurusd_model.pkl
└── data/
    ├── EURUSD_Official_OHLCV.csv
    └── EURUSD_Official_Fetures_Included.csv
```

**Tech stack:** `Python` · `pandas` · `numpy` · `yfinance` · `ta` · `scikit-learn` · `xgboost` · `statsmodels` · `matplotlib` / `seaborn` · `joblib`

**To reproduce the full pipeline (data → features → models):**
```bash
...
**To just use the trained model without rerunning the pipeline:**
```bash
pip install -r requirements.txt
python predict.py
```
